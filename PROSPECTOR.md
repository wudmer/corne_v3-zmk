...
- **Ambient light sensor:** `config/clover_dongle.conf` has
...
- **RGB / ext-power keys on the dongle:** the `extras` layer's `&rgb_ug` and
  `&ext_power` keys are `&trans` in `config/clover_dongle.keymap`, because the
  ...
  the keyboard in this setup. Keep `clover_dongle.keymap` in sync with
  `corne.keymap` when you edit layers.
- **ZMK Studio** now runs on the dongle (`clover_dongle` build has the
